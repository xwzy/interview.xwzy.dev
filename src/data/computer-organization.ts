  ...
  topics: [
    ...
    },
    {
      ...
      questions: [
        ...
        {
          id: 'co-cpu-gpu',
          title: 'GPU 为什么适合深度学习？SIMT、显存层级和「利用率」分别是什么？',
          difficulty: 'advanced',
          tags: ['GPU', 'SIMT', '显存', 'MFU'],
          points: [
            '**与 CPU 的架构分野（一句话立住）**：CPU 是**延迟导向**——深乱序流水线、大缓存、强控制流，用复杂度换单线程快；GPU 是**吞吐导向**——**几千个简单核心（SIMT：同一条指令同时作用于一批线程）**，靠**海量线程并发隐藏访存延迟**（一组等数据，切换另一组算——线程切换零成本是它敢这么设计的前提）。深度学习恰好是"**大规模规则计算**"（矩阵乘为主、分支少、算术密度高），完美匹配 SIMT。',
            '**显存层级（性能调优的地图）**：**寄存器**（每线程私有，最快）→ **共享内存/L1**（线程块内共享，程序员显式管理——kernel 优化的主战场：分块矩阵乘用共享内存复用数据）→ **L2**（全片共享）→ **HBM 显存**（几十 GB、带宽 TB/s 级但延迟数百周期）→ **PCIe/NVLink**（与主机/卡间通信——NVLink 带宽是 PCIe 的数倍，这是训练集群组网的动机之一）。深度学习的性能问题大半是**数据搬运**问题，不是算的问题——与 CPU 的 Cache 局部性哲学同源但层级更深。',
            '**算力墙与带宽墙（进阶必考概念）**：一个算子的**算术强度**（FLOPs / 字节搬运量）决定它卡在哪——矩阵乘是计算密集（吃满算力），LayerNorm/激活/数据搬运是带宽密集（空转算力等数据）；**Roofline 模型**把这个关系画成一条折线，优化就是"把算子从屋顶斜线段推到平顶段"——能说出"这个模型是 bandwidth bound"就超过了背参数的候选人。',
            '**MFU（模型算力利用率）是训练工程的核心指标**：实际吞吐 FLOPs / 峰值算力——主流大模型训练 MFU 在 30~60% 之间；损失来源清单：通信等待（AllReduce 与计算不重叠）、数据加载、kernel 融合不足（小算子发射开销）、TPOT 占比高的访存算子；优化手段（与分布式训练题分工：那题讲切分策略，本题讲单卡视角）：**算子融合（flash attention 类）**、混合精度（TF32/BF16 翻倍算力）、PDL/编译器（XLA/Triton）生成融合 kernel。',
            '收束格局：理解 GPU 的钥匙是"**用并发隐藏延迟，程序员管理数据搬运**"——推理与训练优化的本质都是让数据流喂得上几千个核心的胃口；横向一句：训练在数据中心 GPU（NVLink 互联），推理与端侧走 NPU/消费卡（与移动端方向的端侧 AI 题衔接），同一套 SIMT/带宽思想。',
          ],
          followUps: [
            {
              question: '为什么矩阵乘法在 GPU 上能接近峰值算力，而 LayerNorm 就不行？',
              points: [
                '算术强度的差别：矩阵乘 `C=A×B` 中每个元素要读 2n 个数、做 n 次乘加——**计算量随 n 增长快于数据量**（O(n³) vs O(n²)），算术强度高，且可用共享内存分块复用数据（一块 A 用 n 次）→ 推到算力平顶段；LayerNorm/Softmax 每读一个数只做 O(1)~O(n) 的少量计算，**搬运 dominates**——无论怎么写都在带宽斜线段。',
                '优化思路因此完全不同：矩阵乘调 tile 大小与寄存器分块（让乘累加单元吃满）；访存类算子只能**融合**（把多个 element-wise 算子合并成一次读写——这也是编译器与 FlashAttention 的核心价值：减少往返 HBM 的次数）。能按"算子的算术强度决定优化手段"分类讨论，说明 roofline 不是背的。',
              ],
            },
          ],
        },
