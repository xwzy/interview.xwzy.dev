  ...
  topics: [
    {
      ...
      questions: [
        ...
        {
          id: 'ops-linux-raid',
          title: 'RAID 0/1/5/10 分别是什么？数据库服务器怎么选？',
          difficulty: 'basic',
          tags: ['RAID', '磁盘', '存储'],
          points: [
            '**四种经典级别（一张账记清）**：**RAID 0**（条带化，数据拆块分布在多盘——**容量 100%、性能最高、零冗余**，一块坏全完）；**RAID 1**（镜像，两盘互拷——容量 50%、写放大、可靠性高）；**RAID 5**（分布式奇偶校验，至少 3 盘——容量 (n-1)/n、**坏一块靠校验重建**、写要算奇偶有惩罚）；**RAID 10**（先镜像后条带，至少 4 盘——容量 50%、性能与可靠性双高、贵）——奇偶校验的数学基础就是**异或**（与位运算题呼应：任意两块数据异或出校验，坏一块用剩下的异或还原）。',
            '**选型口径（按负载特征对号）**：**数据库（MySQL/PG）**：**RAID 10 是标配**（随机读写都吃性能、redo/binlog 顺序写密集——RAID 5 的写惩罚在这种负载下是慢性病，且重建窗口长风险大）；**归档/备份/文件服务器**（读多写少、容量敏感）：RAID 5/6（RAID 6 双校验——大容量时代单盘重建期间再坏一块的恐惧是真实的）；**纯性能可重建**（缓存放的临时数据、消息队列的可以重建场景）：RAID 0 甚至单盘 + 多副本（**分布式时代"冗余上移"**：Kafka/HDFS 自己做副本，单机 RAID 的必要性下降——这个视角是加分点）。',
            '**现代语境的清醒认知（别当九几年答题）**：云时代**云盘（EBS 类）已经在存储层做了多副本**，实例层面看到的"一块盘"背后是三副本+纠删码——**用户自己再配 RAID 的场景大幅减少**（本地盘实例/自建机房仍用）；**RAID ≠ 备份**（误删、火灾、逻辑损坏 RAID 都救不了——备份是另一件事，见 RTO/RPO 的容灾分层）；**BBU/缓存**（RAID 卡写缓存 + 电池保电——断电时缓存数据不丢，数据库 fsync 语义依赖它——与文件系统的 fsync 题衔接）。',
            '答这题的层次：报得出各级账目（容量/性能/容错）→ 会按负载选（数据库 RAID 10 的理由说得出口）→ 知道时代的位移（分布式冗余上移、RAID 不是备份）——三层都到，一道 basic 题答出资深感。',
          ],
          followUps: [
            {
              question: 'RAID 5 坏了一块盘，换盘重建期间系统是什么状态？为什么说大容量盘让 RAID 5 变危险了？',
              points: [
                '**降级运行（degraded）**：坏一块后 RAID 5 靠校验仍能读写（读要现算校验、写要更新校验——性能下降）；换新盘触发**重建**（全盘校验扫描重算——**几 TB 盘要数小时到一天**，期间磁盘满负荷、剩余盘承担重建压力、**再坏一块 = 数据全失**（RAID 5 只容忍单盘故障）。',
                '大容量时代的风险数学：单盘容量涨（重建时间线性变长）× 重建期间整组盘高负载（同龄盘同时购入，故障概率相关而非独立）× URE（不可恢复读错误：重建时读到坏扇区的概率随容量上升）——三者叠加让"大盘 RAID 5"的实际可靠性远低于理论；所以现代选择：重要数据 RAID 10 或 RAID 6，或者干脆**分布式多副本**把单点风险分散掉——能算这笔风险账，这题就透了。',
              ],
            },
          ],
        },
    ...
    },
    {
      ...
      questions: [
        ...
        {
          id: 'ops-platform-engineering',
          title: '平台工程是什么？怎么建设内部开发者平台（IDP）避免沦为「工具堆砌」？',
          difficulty: 'advanced',
          tags: ['平台工程', 'IDP', '黄金路径', '开发者体验'],
          points: [
            '**定位先说清（与 DevOps/SRE 的关系）**：DevOps 是文化运动（打破开发运维墙），但"开发者要自助做所有事"把**认知负荷**全压给了每个工程师——平台工程是组织规模化后的回摆：**建一个产品化的内部平台（IDP），把重复的云原生/CI/CD/可观测/合规能力封装成自助服务**，开发者专注业务交付。一句话：**DevOps 说"你们都是运维"，平台工程说"我们做运维产品给开发者用"**——Gartner 预测 80% 工程组织会设平台团队，这不是新潮词是组织演化的必然形态。',
            '**IDP 的核心构件（答题骨架）**：**服务目录**（所有服务/组件/所有者的单页画像——谁负责、跑在哪、SLO 多少、依赖谁；Backstage 是事实标准框架）、**软件模板与黄金路径**（Golden Path：新建一个服务 = 填模板一键生成**代码骨架 + CI/CD + 监控 + 安全基线**——用脚手架把组织的最佳实践"焊死"在起点上，走正确的路是最容易的路）、**自助操作**（申请资源/权限/环境自助化——审批流代替工单），外围：统一的可观测入口、文档与推荐实践。',
            '**「平台即产品」是成败分水岭（本题的深水区）**：平台的第一定律——**开发者没有义务用你**（他们可以用 kubectl/helm/云控制台绕过你）；所以 IDP 要按产品运营：**用户研究**（开发者痛点访谈，别拍脑袋加功能）、**采用率与满意度度量**（NPS、黄金路径的使用率、"绕过率"）、**迭代节奏**（像 toC 产品一样排期）——反面案例遍地：堆了一堆组件的"平台部"没人用，沦为成本中心。"工具堆砌"与"平台"的区别就是**有没有用户视角和采用闭环**。',
            '**度量与边界（务实收束）**：效能度量用 **DORA 四指标**（部署频率、变更前置时间、变更失败率、MTTR——度量平台对交付的影响）+ **SPACE**（满意度/性能/活动/沟通/效率——个人维度），警惕把度量变成 KPI 监视（度量平台不度量个人——与稳定性题的"指标用于改进不是考核"同一条伦理）；建设边界：**从小而痛的场景切入**（部署自助化通常是最好的第一刀），平台团队按产品配比（少量平台工程师服务大量团队——杠杆率就是平台的价值）；不要追求大而全——**没被使用的能力是负资产**。',
          ],
          followUps: [
            {
              question: '怎么说服团队用 IDP 而不是继续自己的脚本和习惯？',
              points: [
                '**让正确的事成为最省力的事**（比说服更有力）：黄金路径生成的新服务 5 分钟可部署 vs 手工搭半天；迁移成本用工具抹平（一键导入存量服务的目录与流水线）；先把**最痛的高频操作**做成丝滑体验（部署/查日志/开环境），用体验拉进来而不是行政命令。',
                '组织侧配套：新服务强制走模板（增量锁死）、存量服务给迁移红利（接入后自动获得监控与成本看板）；**用采用率数据说话**（60% 团队自助部署、上线周期从两周到两天）向管理层证明投入产出——推广平台与推广任何内部产品是同一门学问（与测试平台题的推动策略互为印证）。',
              ],
            },
          ],
        },
