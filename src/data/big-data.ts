  ...
  topics: [
    ...
    },
    {
      ...
      questions: [
        ...
        {
          id: 'bd-warehouse-metrics',
          title: '指标体系怎么从零搭建？原子指标、派生指标、北极星指标分别是什么？',
          difficulty: 'intermediate',
          tags: ['指标体系', '北极星指标', '数据治理'],
          points: [
            '**指标分层命名**：**原子指标**（业务过程 + 度量：下单金额）、**派生指标**（原子 + 修饰词 + 时间粒度：近 7 天移动端下单金额）、**复合指标**（比率/占比/差值：客单价 = 金额/单量）；统一命名规范（时间 + 修饰词 + 原子词 + 度量）并登记**指标字典**（口径、负责人、更新频率）——与分层题的口径治理衔接：字典就是口径唯一的载体。',
            '建法两流派（答题给框架）：**OSM**（目标 Objective → 策略 Strategy → 度量 Metric：从业务目标反推该看什么，避免拍脑袋堆指标）与**北极星 + 指标树**（一个核心成功度量，逐层拆解为可归因、可分工的输入指标树）；北极星的真正作用是**对齐**——每个团队在树上找到自己能影响的节点，而不是各看各的仪表盘。',
            '**好指标的标准（体现思考深度）**：可解释（涨跌能归因到维度）、可行动（变动能指导动作）、**抗操纵**（考核点击量就会诞生标题党——古德哈特定律在指标体系的体现）；核心指标必须配**护栏指标**（体验/成本/质量类），防止"北极星上涨是拿未来换的"（例：推送打开率涨 + 卸载率也涨）。',
            '落地工程：指标定义沉淀为**语义层/统一 SQL 模块**（One Metric Everywhere——BI、API、AB 实验平台同一口径，消灭"同名不同义"）；指标生命周期管理（评审上线 → 使用度监控 → 废弃归档），**口径变更必须广播下游**并留切换记录。',
            '答题结构收束：业务目标 → 北极星 → 指标树（每层 2~3 个具体指标）→ 治理机制；**没有目标指向的指标罗列是仪表盘堆砌，不是指标体系**——先给目标再给指标，顺序反了就是零分答案。',
          ],
          followUps: [
            {
              question: '核心指标涨了 5% 但没人知道为什么，你的归因思路？',
              points: [
                '三层下钻：**维度分解**（渠道/地区/新老用户/平台逐维拆，看涨幅集中在哪个切片——加法指标算贡献度，乘法指标用对数分解/权重法定位主贡献因子）；**同期群对比**（新用户 vs 老用户、这个 cohort 带来的还是存量行为变化）；**变更对照**（当天有没有发版/活动/外部事件——先排除"我们做了什么"再谈自然波动）。',
                '沉淀：归因结论回写指标文档（这个指标历史上对什么敏感），让下一次涨跌有先验；归因能力本身就是指标体系设计质量的检验——树拆得好，每一层都能回答"为什么"，拆不好就只能说"总体涨了"。',
              ],
            },
          ],
        },
    ...
    },
    {
      ...
      questions: [
        ...
        {
          id: 'bd-flink-dim-join',
          title: 'Flink 实时维表关联（lookup join）怎么做？怎么避免高 QPS 打爆维表存储？',
          difficulty: 'intermediate',
          tags: ['Flink', '维表关联', 'lookup join'],
          points: [
            '场景：事实流（订单、曝光）要补充维表信息（用户画像、商品类目、门店地址），维表在 MySQL/Redis/HBase；Flink 的 **lookup join** 是每条流记录按 key 同步点查外部维表——**延迟与外部存储压力都挂在主链路上**，这道题考的就是这层的工程化。',
            '**三层加速（答题主线）**：① **缓存**（lookup cache：ALL 模式全量缓存适合小维表，代价是刷新周期内的数据延迟；LRU + TTL 适合大维表）；② **异步 I/O**（AsyncFunction 并发发出多个查询，不把算子 slot 卡在同步等待上，配超时与重试）；③ **广播状态**（维表变更流 broadcast 到所有并行度，内存里直接 join——零外部查询，适合小且低频变更的维表，是"打爆问题"的根治解）。三层按 维表大小 × 变更频率 选型，能画这张决策表就是满分结构。',
            '**一致性语义（高级考点）**：缓存与广播里的维表都是**某个时间点的快照**——流关联到的是"当时"还是"最新"？业务要 latest（直接用，容忍短延迟）还是要 **as-of**（按事实的事件时间取当时的维度版本——财务、审计口径必需）：as-of 要维表版本化（拉链表，见数仓方向）或 Flink 的 temporal table join（版本表 + 事件时间区间匹配）。能主动区分这两种语义，说明真踩过口径问题。',
            '兜底与治理：维表查询**限流 + 降级**（查不到给默认维度，事后补偿修正，别让维表单点拖死整条作业——反压会联动 checkpoint 超时）；缓存命中率监控进大盘（命中率跌 = 维表变更频繁或 key 长尾，处理策略完全不同）；维表存储选型按 QPS 升级（MySQL → Redis/HBase → 广播）。',
          ],
          followUps: [
            {
              question: '维表几万行、每天凌晨全量更新一次，最优方案是什么？为什么？',
              points: [
                '**广播状态 + 定时全量刷新**（或 ALL cache 短 TTL）：维表整体进每个并行度的内存，join 变成纯内存操作——**零外部查询、无击穿风险**，凌晨更新后广播新版本（双缓冲切换避免刷新期间的半新半旧）。',
                '为什么不用 Redis 缓存方案：几万行维表的查询 QPS 与流对齐（百万级），即便缓存命中 95% 仍有数万 QPS 打到外部存储，且多了网络序列化开销——**能装进内存的维表，内存就是最优存储**；判断标准回到"维表大小 × 变更频率 × 流量"三要素，而不是无脑上中间件。',
              ],
            },
            {
              question: 'temporal table join 的 as-of 语义具体怎么实现？',
              points: [
                '两个要件：**版本化维表**（primary key + changelog 流，或拉链表的 start/end 区间）+ **事实的事件时间**；Flink 按事件时间在维表历史版本里二分定位"当时的值"，水位线控制版本可见性与结果正确性（维表更新事件必须先于事实事件被处理到——并行度下要靠 watermark 对齐保证）。',
                '与拉链表的等价性：没有版本表时，把维表做成拉链，事实流按 `dt BETWEEN start AND end` 区间 join 是同一语义的手工实现——能把 Flink SQL 语法与数仓的物理设计对上，说明两个方向的知识是通的。',
              ],
            },
          ],
        },
