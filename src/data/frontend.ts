  ...
  topics: [
    ...
    },
    {
      ...
      questions: [
        ...
        {
          id: 'fe-eng-dataviz',
          title: '前端数据可视化怎么选型？Canvas、SVG、WebGL 各适合什么？大数据量图表怎么优化？',
          difficulty: 'intermediate',
          tags: ['数据可视化', 'Canvas', 'SVG', '性能'],
          points: [
            '**三种渲染技术的本质区别（选型的根）**：**SVG**（保留模式——每个图形是 DOM 节点：**事件与无障碍免费获得**、可 CSS/DevTools 调试；但**上千元素后 DOM 开销爆炸**，重绘成本高）；**Canvas**（立即模式——一帧画布一张图：**万级元素依然流畅**、内存占用小；代价是没有对象——命中检测要自己做（isPointInPath/数学计算）、无 DOM 无障碍、改动要重绘）；**WebGL/WebGPU**（GPU 并行——**十万百万级点、3D、复杂 shader**；开发成本最高）。速记：**几百元素 SVG、几万 Canvas、十万以上 WebGL**。', 
            '**图表库选型（站在轮子上）**：**ECharts**（功能最全、中文生态、大数据量内置优化（large 模式、采样）——国内中后台事实标准）；**AntV 系**（G2 图形语法理念、L7 地理——数据驱动型产品）；**D3**（不是图表库是**可视化内核**——任意定制用它，学习曲线陡）；轻量场景 Chart.js；React 生态的 visx/Recharts。选型口径：**业务图表用成熟库（配置出活）+ 特殊可视化才 D3/自绘**——能用配置解决的不写代码。', 
            '**大数据量图表的优化阶梯（性能题眼）**：① **数据降采样**（一屏像素就一千多个点，画十万点是浪费——LTTB 算法保形状降点、按时间窗口聚合 min/max/avg）；② **增量渲染**（实时曲线只画新段不清屏——Canvas 的 retained 技巧）；③ **分层 Canvas**（静态轴/网格一层不动，数据层重绘——减少重画面积）；④ **离屏计算**（数据变换放 Web Worker，主线程只画——与长任务优化题呼应）；⑤ **交互降级**（拖动缩放时显示采样点，松手再精化）。监控指标：**帧率 + 首屏渲染时长 + 内存**（图表是内存泄漏重灾区——绑定与监听要随组件销毁）。', 
            '**可视化的表达层（超出"画得快"的加分）**：**图例与交互**（tooltip/legend 联动、缩放平移的边界手感）；**无障碍**（SVG 可加 aria、Canvas 提供**数据表格降级**——可访问图表是多数团队空白）；**响应式**（容器查询适配 + 小屏的图表简化策略——与容器查询题衔接）；**颜色的责任**（色盲友好配色、语义色一致——可视化是"把数据翻译成感知"，色板是这个翻译的词汇表）。',
          ],
          followUps: [
            {
              question: '实时监控大盘（每秒刷新、几十条曲线、每条上万点）页面越来越卡，怎么排查优化？',
              points: [
                '分层定位先看**渲染路径**：Profiler 确认卡在 JS 计算（数据变换/图表库 layout）还是渲染（重绘面积）；常见根因三连——**全量重绘**（每秒 setData 全图重置——改为增量 append）、**数据没降采样**（万点曲线 × 几十条 = 每秒百万级路径计算）、**多图表同步重算**（一条推送触发所有图各自处理——数据中心化，Worker 里统一变换分发）。', 
                '方案组装：数据层（Worker 变换 + 环形缓冲区只留窗口数据）、渲染层（分层 Canvas + 增量绘制 + 视觉降低样：滚动时降采样、停顿时精化）、刷新策略（requestAnimationFrame 节流合并一帧内多次推送、不可见 tab 暂停——visibilitychange）；把"每秒推送"与"每帧渲染"**解耦成生产消费**——这题本质是流式渲染版的虚拟列表思想：**可视窗口内的数据才值得画**。',
              ],
            },
          ],
        },
        ...
        {
          id: 'fe-eng-permission',
          title: '中后台的前端权限怎么做？路由、菜单、按钮、接口权限分别在哪一层管？',
          difficulty: 'intermediate',
          tags: ['前端权限', '动态路由', 'RBAC', '指令'],
          points: [
            '**四层权限各归其位（答题主线）**：① **接口权限**（安全边界——token/权限码由**后端强制校验**，前端只是提示体验；401/403 拦截器统一跳转，这一层是底线：前端藏按钮≠没有权限，**绕过 UI 直接调接口必须被后端拦**）；② **路由权限**（能访问哪些页面）；③ **菜单权限**（侧边栏显示——通常路由权限的展示投影）；④ **按钮/操作权限**（页面内增删改查按钮的显隐）。与系统设计方向 RBAC 题分工：那题讲后端模型（角色—权限—数据范围），本题讲前端消费侧。',
            '**路由权限的两种实现（必考对比）**：**动态路由**（登录后按权限接口拉取菜单/权限码 → **前端过滤本地路由表**（meta.permissions 标注）或后端直接下发路由 JSON → `addRoute` 动态注册——菜单后端下发的好处：权限变更不用发版；风险：路由数据要防注入校验）；**全局守卫**（路由全量注册，`router.beforeEach` 里按 meta 校验权限码，无权跳 403——实现简单、路由结构在代码里可维护）；实践常混合：静态核心路由（登录/404）+ 动态业务路由。',
            '**按钮级权限的惯用法**：**自定义指令**（`v-permission=[\'order:delete\']`——无权时移除元素，实现： mounted 时比对权限码，无权 el.remove 或置 disabled，注意 remove 后布局塌陷的体验）；或**组件封装**（`<Auth code="xx">` 包裹，支持 slot 降级成"无权占位"）；权限码命名规范（`资源:操作` 如 `order:delete`）要与后端接口的权限标识**同源**——一套码两端用，避免对不上的维护灾难。',
            '**工程化细节（做过才答得出）**：权限码的**类型安全**（TS 枚举/常量集中管理，杜绝魔法字符串）；**权限变更的生效**（登录时拉取 + 可主动刷新（切换角色/权限升级）——路由要 reset 重算）；**缓存与回退**（权限接口失败用上次的缓存保底可用性，但要标注过期）；**埋点联动**（无权按钮的曝光点击埋点——帮产品发现"用户想要但没权限"的需求）。',
            '收束：前端权限的本质是"**体验层裁剪 + 引导**"，安全永远在后端；衡量做得好的标准——权限码与后端同源、变更即时生效、按钮/路由/菜单三层来自同一份权限数据（单一数据源），而不是三处各写一套 if。',
          ],
          followUps: [
            {
              question: '后端下发的动态路由 JSON，前端怎么消费才安全又优雅？',
              points: [
                '消费链：后端下发**结构化描述**（path/name/component 标识/权限码）→ 前端维护 **component 映射表**（`componentMap: { \'system/user\': () => import(...) }`——下发标识而不是函数字符串，**杜绝 eval/new Function 执行任意代码**）；递归转换成路由对象（处理嵌套布局/重定向）；`addRoute` 注册后 **`router.replace` 触发一次导航**让新路由生效。',
                '边界清单：刷新页面后动态路由丢失（守卫里"未拉权限先拉再放行"的 pending 队列处理——Vue Router 的经典坑，next 触发时序）；404 要动态注册**之后**再注册（否则刷新即 404）；菜单图标的动态渲染（下发 icon 标识映射本地图标组件）；这些坑点全是实践过的信号。',
              ],
            },
          ],
        },
