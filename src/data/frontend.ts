  ...
  topics: [
    ...
    },
    {
      ...
      questions: [
        ...
        {
          id: 'fe-coding-lru-cache',
          title: '手写 LRU 缓存',
          difficulty: 'advanced',
          tags: ['手写', 'LRU', '缓存'],
          points: [
            '**LRU（最近最少使用）**：容量满时淘汰最久未访问的项；get/put 都算"使用"，要求 get/put 都是 **O(1)**。',
            '**经典数据结构**：哈希表 + 双向链表——哈希表定位 O(1)，链表维护访问顺序（头最新、尾最旧），淘汰从尾部摘除；用**哨兵节点**简化边界处理。',
            '**JS 特权解法**：`Map` 迭代顺序即插入顺序——get 时 delete 再 set 即"移到最新"，put 后超容量删第一个 key，几行实现 O(1) LRU；面试建议先讲双向链表原理，再给 Map 版本。',
            '扩展：put 更新已存在 key 时也要刷新"最近使用"；可加 TTL 过期（缓存项存过期时间戳，get 时惰性删除 + 定期清理）。',
            '```ts\nclass LRUCache<K, V> {\n  private map = new Map<K, V>()\n\n  constructor(private capacity: number) {}\n\n  get(key: K): V | undefined {\n    if (!this.map.has(key)) return undefined\n    const value = this.map.get(key)!\n    this.map.delete(key) // 重新插入 = 移到最新位置\n    this.map.set(key, value)\n    return value\n  }\n\n  put(key: K, value: V) {\n    if (this.map.has(key)) this.map.delete(key)\n    this.map.set(key, value)\n    if (this.map.size > this.capacity) {\n      // Map 的第一个 key 就是最久未使用的\n      const oldest = this.map.keys().next().value\n      if (oldest !== undefined) this.map.delete(oldest)\n    }\n  }\n}\n```',
          ],
          followUps: [
            {
              question: '用双向链表实现时为什么要哨兵节点？',
              points: [
                '哨兵（dummy head/tail）让**头尾永远存在**：插入、删除、淘汰都统一为"已知前驱后继的摘除"，不需要对空表、单节点、头节点、尾节点各写分支。',
                '没有哨兵时至少要处理四种边界，容易漏；O(1) 的 get/put 靠哈希表定位 + 节点 prev/next 指针维护访问顺序。',
              ],
            },
            {
              question: 'LRU 和 LFU 的区别是什么？分别适合什么访问模式？',
              points: [
                '**LRU 按"最近访问时间"淘汰**：对偶发的一次性批量扫描敏感（热点被一次性挤掉）；实现 O(1)（哈希 + 双向链表）。',
                '**LFU 按"访问频率"淘汰**：抗偶发扫描，但对"历史热点"迟钝——过气的老 key 占着坑，需要**频率衰减/分桶老化**；严格 O(1) 要用"频率桶 + 桶内双向链表"。',
                '工程折中：Redis 的近似 LRU（抽样淘汰）与 LFU 的对数衰减模式，都是"理论最优换实现成本"的取舍。',
              ],
            },
          ],
        },
