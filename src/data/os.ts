  ...
  topics: [
    ...
    },
    {
      ...
      questions: [
        ...
        {
          id: 'os-mem-buddy-slab',
          title: '内核怎么管理物理内存？伙伴系统和 slab 分配器分别解决什么问题？',
          difficulty: 'advanced',
          tags: ['物理内存', '伙伴系统', 'slab'],
          points: [
            '先立分层框架：**页分配器（伙伴系统）管"整页"内存，slab 在它之上管"小对象"**——内核自己的数据结构（task_struct、inode、各种描述符）动辄几十几百字节，不可能每次按 4KB 页分配；用户态 malloc（见 malloc 题）则是第三层，在用户库层面把内核给的内存再切成应用对象。三层各管一个粒度。',
            '**伙伴系统（Buddy System）**：以 **2 的幂**为块大小组织空闲页（1/2/4…1024 页）；分配时从最小的满足阶里找，没有就**从更高阶"对半劈"**（另一半挂到低一阶的空闲链表——这就是"伙伴"）；释放时看**伙伴是否也空闲**，是就**合并成大块**继续向上合并。解决的问题：**外碎片**——不合并的话大块请求会被切成满地碎片；代价：内部碎片（要 5 页给 8 页）+ 合并依赖连续地址。',
            '**slab 分配器**：针对**高频分配的同类小对象**——为每种对象类型建一个 **kmem_cache**（对象缓存池），底层从伙伴系统拿整页，切成**等大的对象槽**；核心优化是 **对象复用**——释放的对象不归还页，而是留在**每 CPU 缓存（per-CPU slab）**里等下次分配，**初始化状态都还在**（task_struct 分配后不用重新 memset），且 per-CPU 避免锁竞争。这层思想直接影响了用户态的 tcmalloc/jemalloc（线程本地缓存，见 malloc 题对照）。',
            '工程视角收束：这套体系解释了很多线上现象——**碎片化导致的"内存看着够却分配失败"**（伙伴系统高阶连续页耗尽，`/proc/buddyinfo` 看各阶余量）、**内核态内存占用**（slab 占用看 `slabtop`，inode/dentry 缓存撑大 RSS）、**大页的价值**（THP/hugetlb 减少 TLB miss 与页表开销，与硬件层呼应）。能从"**分层分配器**"的视角把 buddy/slab/malloc 串成一个体系，是操作系统题的顶级答案。',
          ],
          followUps: [
            {
              question: '伙伴系统的空闲页明明够，为什么大块分配还是会失败？怎么排查？',
              points: [
